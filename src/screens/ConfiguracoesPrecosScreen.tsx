  ...
  const salvarPrecos = async () => {
    try {
      ...
      if (isNaN(valorCorteBarba) || valorCorteBarba <= 0) {
        Alert.alert('Atenção', 'Por favor, informe um preço válido para Corte e Barba.');
        return;
      }

      if (isNaN(valorSoCorte) || valorSoCorte <= 0) {
        Alert.alert('Atenção', 'Por favor, informe um preço válido para Só Corte.');
        return;
      }

      if (isNaN(valorSoBarba) || valorSoBarba <= 0) {
        Alert.alert('Atenção', 'Por favor, informe um preço válido para Só Barba.');
        return;
      }
      ...
      // Atualiza os valores globais (para uso em todo o app)
